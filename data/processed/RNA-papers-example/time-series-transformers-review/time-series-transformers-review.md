...
3Department of CSE, MoE Key Lab of Artiﬁcial Intelligence, Shanghai Jiao Tong University
...
adaptations and modiﬁcations that have been made
...
ing, anomaly detection, and classiﬁcation. Empiri-
...
al., 2022; Tuli et al., 2022], and classiﬁcation [Zerveas et
al., 2021; Yang et al., 2021]. Speciﬁcally, seasonality or pe-
...
Benidis et al., 2022; Torres et al., 2021], classiﬁcation [Is-
...
greatly beneﬁt the time series community.
In this paper, we aim to ﬁll the gap by summarizing the
...
modiﬁcations and application domains for time series Trans-
formers. For network modiﬁcations, we discuss the improve-
...
classiﬁcation. For each time series Transformer, we analyze
...
formers and GNN for time series, pre-trained Transformers
arXiv:2202.07125v5  [cs.LG]  11 May 2023

---

## Página 2

for time series, Transformers with architecture level variants,
and Transformers with NAS for time series. To the best of
our knowledge, this is the ﬁrst work to comprehensively and
...
each position which is more ﬂexible [Kenton and others,
2019; Gehring et al., 2017].
Relative Positional Encoding
Following the intuition that pairwise positional relationships
between input elements is more beneﬁcial than positions of
...
the perspectives of network modiﬁcations and application domains.
...
pose a taxonomy from perspectives of network modiﬁcations
...
ﬁcations, we summarize the changes made on both module
...
ing, anomaly detection, and classiﬁcation. In the following
...
formers. A common design is to ﬁrst encode positional infor-
...
Positional Attention Architecture Anomaly
Forecasting Classification
Encoding Module Level Detection
Vanilla Learnable Timestamp Time Series Spatio-Temporal Event
Encoding Encoding Encoding Forecasting Forecasting Forecasting | None |
| --- | --- |
| Positional
Encoding |  |
|  |  |
...
effective.
Compared to ﬁxed vanilla positional encoding,
learned embeddings are more ﬂexible and can adapt to spe-
ciﬁc tasks.
[Zerveas et al., 2021] introduces an embedding
...
plexity of O(N 2) (N is the input time series length), which
becomes the computational bottleneck when dealing with
long sequences. Many efﬁcient Transformers were proposed
to reduce the quadratic complexity that can be classiﬁed into
...
nism, in which nodes at the ﬁnest scale correspond to the orig-
...
a hierarchical architecture also enjoys the beneﬁts of efﬁcient
...
The ﬁrst type of variant for module-level Transformers is to
design new attention modules, which is the category with the
largest proportion. Here we ﬁrst describe six typical works:
...
Quatformer [Chen et al., 2022]
FEDformer [Zhou et al., 2022]
Crossformer [Zhang and Yan, 2023] | O N 2 O N 2 N
...
to O(N log N).
Instead of using explicit sparse bias, In-
...
ity by randomly selecting a ﬁxed-size subset of frequency.
...
Explainable Artiﬁcial Intelligence (XAI). TFT [Lim et al.,
...
mation from various covariates to perform forecasting.
It
...
dure based on variational inference. SSDNet ﬁrst uses Trans-
...
knowledge, Non-stationary Transformer [Liu et al., 2022b] is
...
a two-stage attention layer is used to efﬁciently capture the
...
triangular,variable-speciﬁc patch attention. It has a triangular
tree-type structure as the later input size shrinks exponentially
and a set of variable-speciﬁc parameters making a multi-
...
baseline model’s performance by iteratively reﬁning the fore-
...
casting is necessary and would beneﬁt the whole community.
...
Trafﬁc Transformer [Cai et al., 2020] designs an encoder-
...
Spatial-temporal
Transformer [Xu et al., 2020] for trafﬁc ﬂow forecasting takes
...
et al., 2022] proposes a cuboid attention for efﬁcient space-
...
temporal self-attention module to efﬁciently capture spatial
...
the inﬂuence of historical events and compute the intensity
...
more ﬂexible named attentive neural datalog through time (A-
...
Transformer based architecture also beneﬁts the time se-
...
such as VAEs [Kingma and Welling, 2014] and GANs [Good-
...
anomaly detection. Note that, MT-RVAE is also for multi-
variate time series but with few dimensions or insufﬁcient
close relationships among sequences where the graph neu-
ral network model does not work well. To deal with such
challenge, MT-RVAE modiﬁes the positional encoding mod-
ule and introduces feature-learning module. Instead, GTA
contains a graph convolution structure to model the inﬂuence
propagation process. Similar to MT-RVAE, GTA also consid-
...
5.3
Transformers in Classiﬁcation
Transformer is proved to be effective in various time series
classiﬁcation tasks due to its prominent capability in captur-
...
13 multivariate time series classiﬁcations.
[Rußwurm and
K¨orner, 2020] studied the self-attention based Transformer
for raw optical satellite time series classiﬁcation and obtained
...
that augments classiﬁcation performance, which utilizes at-
tention score for important timestamps masking and recon-
struction and brings superior performance.
Pre-trained Transformers are also investigated in classiﬁca-
tion tasks. [Yuan and Lin, 2020] studies the Transformer for
raw optical satellite image time series classiﬁcation. The au-
...
then ﬁne-tuned in downstream tasks such as classiﬁcation.
[Yang et al., 2021] proposes to use large-scale pre-trained
speech processing model for downstream time series classi-
ﬁcation problems and generates 19 competitive results on 30
popular time series classiﬁcation datasets.
6
...
prolonging input length, various Transformer-based model
...
Model Size Analysis
Before being introduced into the ﬁeld of time series predic-
...
key advantages Transformer holds in these ﬁelds is being able
...
seasonal-trend decomposition model can signiﬁcantly boost
...
promising for future research of Transformers in time series.

### Tabla 1 (Página 6)

| Model | None | Transformer Autoformer Informer Reformer LogFormer |
| --- | --- | --- |
| Input Len | 96
192
336
720
1440 | 0.557 0.239 0.428 0.615 0.667
0.710 0.265 0.385 0.686 0.697
1.078 0.375 1.078 1.359 0.937
1.691 0.315 1.057 1.443 2.153
0.936 0.552 1.898 0.815 0.867 |
...
12
24
48 | 0.557 0.234 0.428 0.597 0.667
0.439 0.282 0.489 0.353 0.387
0.556 0.238 0.779 0.481 0.562
0.580 0.266 0.815 1.109 0.690
0.461 NaN 1.623 OOM 2.992 |
...
data overﬁtting. One of the key features of time series data
...
former can enhance performance signiﬁcantly. Moreover, it
...
series data and characteristics of speciﬁc tasks.
7.2
...
bring not only signiﬁcant performance improvements like in
trafﬁc forecasting [Cai et al., 2020; Xu et al., 2020] and multi-
...
ies mainly focus on time series classiﬁcation [Zerveas et al.,
...
tain the vanilla Transformer’s architecture with modiﬁcations
mainly in the attention module. We might borrow the idea
from Transformer variants in NLP and CV which also have
architecture-level model designs to ﬁt different purposes,
...
ers speciﬁcally optimized for time series data and tasks.
7.5
...
of Transformers. Manual conﬁguring these hyper-parameters
...
discovering both memory- and computational-efﬁcient Trans-
...
| Model | None | FEDformer | Autoformer | Informer | LogTrans | Reformer | Transformer | Promotion |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MSE |  | Ori Decomp Ori Decomp Ori Decomp Ori Decomp Ori Decomp Ori Decomp Relative |  |  |  |  |  |  |
| Out Len | 96
192
336
720 | 0.457 0.203 0.581 0.255 0.365 0.354 0.768 0.231 0.658 0.218 0.604 0.204 53%
0.841 0.269 1.403 0.281 0.533 0.432 0.989 0.378 1.078 0.336 1.060 0.266 62%
1.451 0.325 2.632 0.339 1.363 0.481 1.334 0.362 1.549 0.366 1.413 0.375 75%
3.282 0.421 3.058 0.422 3.379 0.822 3.048 0.539 2.631 0.502 2.672 0.537 82% |  |  |  |  |  |  |

---

## Página 8

[Bl´azquez-Garc´ıa et al., 2021] Ane
Bl´azquez-Garc´ıa,
Angel
...
Sungroh Yoon.
Deep learning for anomaly detection in time-
...
angular, variable-speciﬁc attentions for long sequence multivari-
...
based on loess. Journal of Ofﬁcial Statistics, 6(1):3–73, 1990.
[Dai et al., 2019] Zihang Dai, Zhilin Yang, Yiming Yang, Jaime G.
Carbonell, Quoc V. Le, et al. Transformer-XL: Attentive lan-
guage models beyond a ﬁxed-length context. In ACL, 2019.
...
[Ismail Fawaz et al., 2019] Hassan
Ismail
Fawaz,
Germain
Forestier, Jonathan Weber, Lhassane Idoumghar, and Pierre-
Alain Muller.
Deep learning for time series classiﬁcation: a
...
[Lim and Zohren, 2021] Bryan Lim and Stefan Zohren.
Time-
...
Self-attention for raw optical satellite time series classiﬁcation.
ISPRS J. Photogramm. Remote Sens., 169:421–435, 11 2020.
[Shabani et al., 2023] Amin Shabani, Amir Abdi, Lili Meng, and
Tristan Sylvain. Scaleformer: iterative multi-scale reﬁning trans-
formers for time series forecasting. In ICLR, 2023.
[Shaw et al., 2018] Peter Shaw,
Jakob Uszkoreit,
and Ashish
...
ald Metzler. Efﬁcient transformers: A survey. ACM Computing
Surveys, 55(6):1–28, 2022.
[Torres et al., 2021] Jos´e F. Torres, Dalil Hadjout, Abderrazak Se-
baa, Francisco Mart´ınez- ´Alvarez, and Alicia Troncoso.
Deep
...
Sun. Fast RobustSTL: Efﬁcient and robust seasonal-trend decom-
...
Gao, Weiyao Lin, Guo-Jun Qi, and Hongkai Xiong.
Spatial-
...
eling and applications for temporal point processes.
In KDD,
2019.
[Yang et al., 2021] Chao-Han Huck Yang, Yun-Yun Tsai, and Pin-
Yu Chen.
Voice2series: Reprogramming acoustic models for
time series classiﬁcation. In ICML, 2021.
...
Xu. Are transformers effective for time series forecasting?
In
AAAI, 2023.
[Zerveas et al., 2021] George
Zerveas,
Srideepika
Jayaraman,
...
Shuai Zhang, Jianxin Li, Hui Xiong, and Wancai Zhang.
In-
