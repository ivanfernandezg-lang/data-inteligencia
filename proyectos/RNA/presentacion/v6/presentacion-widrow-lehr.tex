...
\azul{desde cero en NumPy} (\emph{scikit-learn} solo para cargar datos y métricas), con
\texttt{SEED=1990} para reproducibilidad.
\vspace{4pt}
{\footnotesize
...
\azul{H2} & El Perceptrón converge en pasos finitos \emph{solo} si los datos son separables; el $\mu$-LMS converge (en media) a la solución de Wiener y minimiza el MSE aun sin separabilidad. \\
...
\midrule
E1  & Capacidad de almacenamiento del Adaline & Adaline ($\alpha$-LMS) & Binarios aleatorios & \azul{H1} \\
E2  & Fronteras aprendidas \emph{vs.}\ Wiener (separable) & Perceptrón, $\alpha$/$\mu$-LMS & Gaussianos separables & \azul{H2} \\
E3  & Reglas lineales en datos no separables & Perceptrón, $\mu$-LMS, Wiener & Gaussianos solapados & \azul{H2} \\
E4  & Velocidad y estabilidad de convergencia & $\alpha$-LMS, Perceptrón, Mays & Gaussianos separables & \azul{H2} \\
E5  & Capacidad no lineal (XOR) & MRII \emph{vs.}\ MLP-backprop & XOR ($2^2$) & \azul{H3} \\
E6  & Generalización (flores) & MLP-backprop & Iris & \azul{H3} \\
E6b & Generalización (dígitos) & MLP-backprop & MNIST-04 & \azul{H3} \\
E6c & Generalización (química) & MLP-backprop & Wine & \azul{H3} \\
E6d & Generalización (no lineal 2D) & MLP-backprop & two-moons & \azul{H3} \\
...
% ---------- 8. E4 ----------
\begin{frame}{E4 --- Velocidad y estabilidad de convergencia}
\begin{columns}[T]
\column{0.58\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{figuras/fig_convergencia.png}\\
  {\scriptsize Error por época de las reglas de corrección (datos separables).}
\end{center}
\column{0.42\textwidth}
\begin{ideaclave}
\textbf{H2 (convergencia)}: en datos separables las reglas de corrección alcanzan error cero en pasos finitos; la \emph{zona muerta} de Mays estabiliza la trayectoria y el $\alpha$-LMS reduce el error lineal de forma monótona.
\end{ideaclave}
\end{columns}
\end{frame}
...
% ---------- 10. E6 Iris ----------
\begin{frame}{E6 --- Generalización: Iris}
\begin{center}
...
% ---------- 11. E6b MNIST ----------
\begin{frame}{E6b --- Generalización: MNIST-04}
\begin{center}
...
% ---------- 12. E6c Wine ----------
\begin{frame}{E6c --- Generalización: Wine}
\begin{center}
  \includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{figuras/fig_wine.png}\\
  {\scriptsize MLP-backpropagation sobre \textbf{Wine} (13 rasgos químicos, 3 cultivares).}
\end{center}
\begin{ideaclave}
\textbf{H3 confirmada}: $98.1\%$ en test. Con rasgos continuos bien escalados la red logra clases casi perfectamente separables en el espacio interno aprendido.
\end{ideaclave}
\end{frame}

% ---------- 13. E6d two-moons ----------
\begin{frame}{E6d --- Generalización: two-moons}
\begin{columns}[T]
