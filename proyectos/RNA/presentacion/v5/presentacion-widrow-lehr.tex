...
\begin{frame}{Experimentos (E1--E7): qué evalúa cada uno}
{\footnotesize
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}c p{0.5\textwidth} p{0.22\textwidth} c@{}}
\toprule
\textbf{Exp.} & \textbf{Qué evalúa} & \textbf{Datos} & \textbf{Hip.} \\
\midrule
E1 & Capacidad de un Adaline & Binarios aleatorios & \azul{H1} \\
E2 & Reglas lineales: fronteras \emph{vs.}\ Wiener & Gaussianos separables & \azul{H2} \\
E3 & Reglas lineales en datos no separables & Gaussianos no sep. & \azul{H2} \\
E4 & Convergencia de reglas de corrección & Gaussianos separables & \azul{H2} \\
E5 & XOR: MRII \emph{vs.}\ MLP-backprop & XOR ($2^2$) & \azul{H3} \\
E6 & Generalización en datos reales & Iris, Wine, MNIST-04, two-moons & \azul{H3} \\
E7 & Equivalencia del gradiente MRIII $\equiv$ backprop & Red sigmoide (Iris) & \azul{H4} \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\begin{tipbox}
Todo implementado \textbf{desde cero en NumPy} (\texttt{SEED=1990}); los conjuntos de datos se
...
\end{columns}
\vspace{2pt}
\begin{block}{Precisión en test}
\azul{Iris} $91.1\%$ (MLP 4-8-3) \quad\textbullet\quad \azul{Wine} $98.1\%$ (13 rasgos, 3 clases)
\quad\textbullet\quad \azul{MNIST-04} $98.5\%$ \quad\textbullet\quad \azul{two-moons} $93.3\%$.
\end{block}
\end{frame}

\subsection{Resultados}
\begin{frame}{E1: capacidad de Cover --- teoría \emph{vs.} experimento}
\begin{columns}[T]
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.38\textheight,keepaspectratio]{figuras/paper_cover.png}\\
  {\scriptsize Fig.~5 del artículo (curvas teóricas de 1965).}
\end{center}
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.38\textheight,keepaspectratio]{figuras/fig_cover.png}\\
  {\scriptsize E1: mi reconstrucción empírica (Monte Carlo).}
\end{center}
\end{columns}
\vspace{2pt}
\begin{block}{Datos: patrones binarios aleatorios --- \textbf{H1 confirmada}}
Desviación media $\leq 0.034$ entre las curvas empíricas y la fórmula de separabilidad. Se verifica
$C_d=N_w$ (en $N_p/N_w=1$) y $C_s\approx 2N_w$ (probabilidad $0.5$); la transición se vuelve más
abrupta al crecer $N_w$.
\end{block}
\end{frame}

\begin{frame}{E2--E3: reglas lineales \emph{vs.} óptimo de Wiener}
\begin{center}
  \includegraphics[width=\textwidth,height=0.46\textheight,keepaspectratio]{figuras/fig_fronteras.png}\\
  {\scriptsize E2--E3: fronteras aprendidas \emph{vs.} solución de Wiener (datos gaussianos 2D).}
\end{center}
\begin{block}{Datos: gaussianos separables (E2) y no separables (E3) --- \textbf{H2 confirmada}}
El Perceptrón converge en 2 épocas pero queda a $20.6^\circ$ de Wiener; $\mu$-LMS a $3.5^\circ$. En
datos \textbf{no} separables, $\mu$-LMS es el mejor ($79.6\%$ \emph{vs.} Wiener $81.0\%$) y el de
menor varianza.
\end{block}
\end{frame}
...
\begin{center}
  ...
  {\scriptsize \textbf{Datos: XOR} (4 patrones $2^2$, no separable).}
\end{center}
\column{0.42\textwidth}
...
\begin{center}
  ...
  {\scriptsize E6: MLP 4-8-3 en \textbf{Iris} (91.1\% test).}
\end{center}
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{figuras/fig_mriii_bp.png}\\
  {\scriptsize E7: error del gradiente y costo (red sigmoide).}
\end{center}
\end{columns}
