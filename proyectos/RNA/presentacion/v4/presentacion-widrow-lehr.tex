...
\end{center}
\begin{center}
  \includegraphics[width=\textwidth,height=0.22\textheight,keepaspectratio]{figuras/paper_adaline.png}\\
...
Con un preprocesador fijo que expande la entrada a términos polinomiales
...
\column{0.6\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.52\textheight,keepaspectratio]{figuras/paper_cover.png}\\
  {\scriptsize Fig.~5: $P_{\text{sep}}$ vs.\ $N_p/N_w$ para distintos $N_w$.}
\end{center}
\column{0.4\textwidth}
\begin{block}{Lectura de las curvas}
\begin{itemize}
  \item $N_p/N_w = 1$: separación \azul{garantizada}, $C_d=N_w$.
  \item $N_p/N_w = 2$: probabilidad $0.5$, define $C_s\approx 2N_w$.
  \item Al crecer $N_w$ la transición se vuelve \textbf{abrupta}.
\end{itemize}
\end{block}
\begin{ideaclave}
Mi \texttt{fig\_cover} reproduce empíricamente estas curvas: coinciden con la teoría de 1964.
\end{ideaclave}
\end{columns}
\end{frame}
...
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.36\textheight,keepaspectratio]{figuras/paper_madaline1.png}\\
...
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth,height=0.36\textheight,keepaspectratio]{figuras/paper_red_3capas.png}\\
...
\begin{frame}{Experimentos y datasets en detalle}
\begin{columns}[T]
...
\toprule
\textbf{Exp.} & \textbf{Qué evalúa} & \textbf{Hip.} \\
\midrule
E1 & Capacidad de un Adaline con patrones binarios aleatorios & \azul{H1} \\
E2 & Reglas lineales: fronteras \emph{vs.}\ Wiener (separable) & \azul{H2} \\
E3 & Reglas lineales sobre datos \textbf{no} separables & \azul{H2} \\
E4 & Convergencia de reglas de corrección ($\alpha$-LMS, Perc., Mays) & \azul{H2} \\
E5 & XOR: MRII \emph{vs.}\ MLP-backpropagation & \azul{H3} \\
E6 & Generalización sobre \textbf{datasets reales} & \azul{H3} \\
E7 & Equivalencia del gradiente MRIII $\equiv$ backprop & \azul{H4} \\
\bottomrule
\end{tabular}}
\column{0.37\textwidth}
\begin{block}{Conjuntos de datos}
{\scriptsize
\begin{itemize}
  \item \azul{Sintéticos}: binarios (E1), gaussianos sep./no sep.\ (E2--E3), XOR (E5).
  \item \azul{two-moons}: no lineal 2D ($93.3\%$).
  \item \azul{Iris}: 4 rasgos, 3 clases ($91.1\%$).
  \item \azul{Wine}: 13 rasgos, 3 clases ($98.1\%$).
  \item \azul{MNIST-04}: dígitos 0--4 ($98.5\%$).
\end{itemize}}
\end{block}
\end{columns}
\vspace{1pt}
\begin{center}
  \includegraphics[width=\textwidth,height=0.22\textheight,keepaspectratio]{figuras/fig_datasets.png}\\
  {\scriptsize Panorama de los conjuntos de prueba usados en los experimentos.}
\end{center}
\end{frame}
