...
\end{center}
\begin{center}
  \includegraphics[height=2.5cm, keepaspectratio]{figuras/paper_adaline.png}\\
  {\scriptsize Fig.~2 del artículo: el Adaline con entrada de respuesta deseada $d_k$.}
\end{center}
\begin{tipbox}
...
\end{block}
\begin{center}
  \includegraphics[height=2.7cm, keepaspectratio]{figuras/paper_frontera_lineal.png}\\
  {\scriptsize Fig.~4 del artículo: la condición $s=0$ como \emph{recta separadora} en el espacio de patrones.}
\end{center}
...
\end{alertblock}
\begin{center}
  \includegraphics[height=2.9cm, keepaspectratio]{figuras/paper_elipse_xnor.png}\\
  {\scriptsize Fig.~7: frontera \emph{elíptica} (preprocesador cuadrático) que sí resuelve XNOR.}
\end{center}
\end{columns}
\end{frame}
...
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_preproc_poli.png}\\
  {\scriptsize Fig.~6: Adaline con entradas expandidas por un preprocesador polinomial fijo.}
\end{center}
\begin{tipbox}
...
\begin{frame}{Capacidad de Cover: las curvas originales del artículo}
\begin{columns}[T]
\column{0.62\textwidth}
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_cover.png}\\
  {\scriptsize Fig.~5 del artículo: $P_{\text{sep}}$ en función de $N_p/N_w$ para distintos $N_w$.}
\end{center}
\column{0.38\textwidth}
\begin{block}{Lectura de las curvas}
\begin{itemize}
  \item En $N_p/N_w = 1$ (es decir $N_p=N_w$) la separación está \azul{garantizada}: $C_d=N_w$.
  \item En $N_p/N_w = 2$ la probabilidad es $0.5$: define $C_s\approx 2N_w$.
  \item Al crecer $N_w$, la transición se vuelve \textbf{abrupta}: $C_s\approx 2N_w$ es un estimador cada vez más preciso.
\end{itemize}
\end{block}
\begin{ideaclave}
Mi \texttt{fig\_cover} (lámina anterior) reproduce empíricamente estas curvas: coinciden con la teoría de 1964.
\end{ideaclave}
\end{columns}
\end{frame}
...
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_madaline1.png}\\
  {\scriptsize Fig.~15: Madaline~I --- capa de Adalines \emph{signum} + \azul{lógica fija} (mayoría).}
\end{center}
\begin{tipbox}
El ``\emph{job assigner}'' reparte la responsabilidad usando la salida lineal: \emph{load-sharing}.
\end{tipbox}
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_red_3capas.png}\\
  {\scriptsize Fig.~11: red \emph{feedforward} de tres capas totalmente conectada.}
\end{center}
\begin{alertblock}{El salto conceptual}
En Madaline~I \textbf{solo la primera capa} es adaptativa; en la red feedforward moderna lo son
\azul{todas}. Falta resolver el error de las capas ocultas.
\end{alertblock}
\end{columns}
\end{frame}
...
\end{center}
\begin{center}
  \includegraphics[height=2.3cm, keepaspectratio]{figuras/paper_taxonomia.png}\\
  {\scriptsize Fig.~33 original del artículo (para contraste).}
\end{center}
\end{columns}
\end{frame}
...
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_perceptron_rosenblatt.png}\\
...
\end{block}
\begin{center}
  \includegraphics[height=3.2cm, keepaspectratio]{figuras/paper_mrii.png}\\
  {\scriptsize Fig.~16 del artículo: arquitectura MRII de dos capas; la perturbación $\Delta s$ prueba el \emph{flip} de cada Adalina.}
\end{center}
...
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_mse_lineal.png}\\
  {\scriptsize Fig.~22: error \azul{lineal}.}
\end{center}
\column{0.34\textwidth}
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_mse_sigmoid.png}\\
  {\scriptsize Fig.~23: error \azul{sigmoide}.}
\end{center}
\column{0.34\textwidth}
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_mse_signum.png}\\
  {\scriptsize Fig.~24: error \azul{signum}.}
\end{center}
\end{columns}
\vspace{2pt}
\begin{ideaclave}
Las tres superficies provienen del \textbf{mismo} Adaline de dos pesos, variando solo la no
linealidad de salida. El paraboloide liso (lineal) se degrada a un tazón suave con mínimos locales
(sigmoide) y finalmente a un relieve \textbf{escalonado} intratable por gradiente (signum). Es la
justificación visual de por qué backprop y MRIII usan sigmoides.
\end{ideaclave}
\end{frame}
...
\begin{center}
  \includegraphics[width=0.96\textwidth, keepaspectratio]{figuras/paper_bp_red.png}
\end{center}
\vspace{-2pt}
\begin{columns}[T]
\column{0.5\textwidth}
...
barrido \azul{hacia atrás} que transporta los $\delta$.
\end{tipbox}
\column{0.5\textwidth}
\begin{block}{Simetría de costo}
El barrido hacia atrás tiene \textbf{la misma complejidad} que el hacia adelante: mismas
evaluaciones de función, sin multiplicaciones de peso en la primera capa.
\end{block}
\end{columns}
\end{frame}
...
\column{0.5\textwidth}
\begin{center}
  \includegraphics[width=\textwidth, keepaspectratio]{figuras/paper_bp_detalle.png}\\
  {\scriptsize Fig.~26: combinador lineal con su circuitería de retropropagación y actualización LMS.}
\end{center}
\column{0.5\textwidth}
\begin{block}{Qué muestra el detalle}
\begin{itemize}
  \item Cada Adalina guarda una \azul{copia de sus pesos} para el camino de retorno.
  \item El bloque \textbf{LMS} aplica $\Delta W = 2\mu\,\delta\,X$ una vez que llega $\delta$ desde aguas abajo.
  \item Los pesos deben transportar señal en \textbf{ambos sentidos} $\Rightarrow$ difícil en hardware analógico.
\end{itemize}
\end{block}
\begin{ideaclave}
Justo esta complejidad de doble sentido es la que \textbf{MRIII elimina} usando perturbación.
\end{ideaclave}
\end{columns}
\end{frame}
...
\end{columns}
\begin{center}
  \includegraphics[height=3cm, keepaspectratio]{figuras/paper_mriii_red.png}\\
  {\scriptsize Fig.~28: arquitectura MRIII. Idéntica a MRII (Fig.~16) pero con \emph{sigmoides}: por eso su gradiente coincide con backprop.}
\end{center}
\end{frame}
