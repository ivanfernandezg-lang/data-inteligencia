"""
Parte 3: Agrega Madaline II, MRIII y experimentos E1-E7 al notebook.
"""
...
class MadalineII:
    ...
    def fit(self, X, d, max_epochs=150, alpha_out=0.5, patience=15,
                ...
                if not accepted and len(flat_ada) >= 2:
                    ...
                    for (l1, j1, _), (l2, j2, _) in itertools.combinations(flat_ada, 2):
                        if l1 == l2:
                            ...
                            # Flip secuencial multcapa
                            y_temp, _ = self.forward(xk, flip=(l1, [j1]))
        ...
        self.W = best_W
        return {'hamming_history': np.array(hamming_hist),
...
print("Funciones MRIII y backprop_gradient definidas.")
""")

# ── CELDA 14: Seccion Experimentos ──
md("""---
## Experimentos
...
Los resultados se referencian desde el informe con `\\figura` y `\\tabla`.""")
...
def check_separable(X, d):
    """Test de separabilidad lineal via LP: d_i * (x_i . w) >= 1, hiperplano homogeneo."""
    N, n = X.shape
    # Variables: w (n weights), sin bias para test de Cover
    # Restricciones: d_i * (x_i . w) >= 1
    c = np.zeros(n)
    A_ub = -d.reshape(-1, 1) * X  # -d_i * x_i . w <= -1
    b_ub = -np.ones(N)
...
for Nw in Nw_list:
    ax.plot(ratios, results[Nw], 'o-', color=colors[Nw], markersize=4,
            label=f'Nw={Nw} (empirico, {n_trials} trials)')
...
# Desviacion empirica vs teorica
for Nw in Nw_list:
...
# Entrenar los 3 algoritmos en dataset separable
...
print(f"Angulos respecto a W*: Perceptron={ang_p:.1f} deg, alpha-LMS={ang_a:.1f} deg, mu-LMS={ang_m:.1f} deg")

# Calcular tr[R] y mu_max teorico
Xb = add_bias(X_sep)
R = Xb.T @ Xb / len(Xb)
trR = np.trace(R)
mu_max_theory = 1.0 / trR
mse_wiener = mse_loss(d_sep, np.dot(Xb, W_wiener))
print(f"tr[R]={trR:.4f}, mu_max teorico=1/tr[R]={mu_max_theory:.6f}")
print(f"MSE de Wiener={mse_wiener:.6f}")
...
axes[0].set_title('Dataset separable')
axes[0].legend()
# No separable
...
def plot_boundary(ax, W, X, d, label, color, ls='-'):
...
for i, (ax, W, title, c) in enumerate(zip(axes, Ws, titles, colors_b)):
    ...
    plot_boundary(ax, W, X_sep, d_sep, title, c)
    plot_boundary(ax, W_wiener, X_sep, d_sep, 'Wiener', 'black', '--')
    ax.set_title(f'{title} ({ang_p if i==0 else ang_a if i==1 else ang_m:.1f} deg vs Wiener)')
    ax.grid(True, alpha=0.3)
...
# Estadisticas ultimas 40 epocas
...
# Tabla lineales
tab = r"""\\begin{tabular}{@{}lccc@{}}
\\toprule
Algoritmo & Acc. separable & Epocas converg. & Acc. no separable \\\\
\\midrule
Perceptron & 100.0\\%% & """ + f"{hp['epochs_to_converge']}" + r""" & """ + f"{pa_m*100:.1f}\\%% $\\pm${pa_s*100:.1f}" + r""" \\\\
$\\alpha$-LMS & 100.0\\%% & -- & """ + f"{aa_m*100:.1f}\\%% $\\pm${aa_s*100:.1f}" + r""" \\\\
$\\mu$-LMS & 100.0\\%% & -- & """ + f"{ma_m*100:.1f}\\%% $\\pm${ma_s*100:.1f}" + r""" \\\\
\\midrule
Wiener (optimo) & 100.0\\%% & -- & """ + f"{acc_wiener_ns*100:.1f}\\%%" + r""" \\\\
\\bottomrule
\\end{tabular}
"""
